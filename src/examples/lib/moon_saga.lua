...
local FORK = function (data, proccess_id)
  ...
  local forked_iterator_params = { proccess_id, unpack(args) };
  iterate(forked_coroutine, unpack(forked_iterator_params));
end
...
iterate = function(...)
  ...
  if (type(yielded_helper) == 'table' and yielded_helper.moon_effect ~= nil) then
    if (yielded_helper.moon_effect == 'moon_saga.TAKE') then
